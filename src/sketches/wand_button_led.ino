...
/* Includes ---------------------------------------------------------------- */
#include <Jazmyn-lab4_inferencing.h>
...
#define BUTTON_PIN D1  // D1
#define RED_PIN D7     // D7
#define GREEN_PIN D9   // D9
#define BLUE_PIN D6    // D6
...
void capture_accelerometer_data() {
    if (millis() - last_sample_time >= SAMPLE_RATE_MS) {
        ...
        if (millis() - capture_start_time >= CAPTURE_DURATION_MS) {
            ...
            run_inference();
            delay(800);
            setLEDColor(false, false, false); // Turn off LED after result
        }
    }
}
...
void loop()
{
    // Button logic: trigger capture on button press
